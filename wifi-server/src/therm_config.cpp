...
bool saveConfig(TempConfig* cfg)
{
	...
	for (uint8_t i = 0; i < cfg->size; i++)
	{
		...
		doc["temp_schedule"][i]["schedule"] = doc.createNestedObject("schedule");

		JsonObject start = doc["temp_schedule"][i]["schedule"].createNestedObject("start");
		JsonObject end = doc["temp_schedule"][i]["schedule"].createNestedObject("end");
	...
	// if (LittleFS.exists(CONF_FILE))	LittleFS.remove(CONF_FILE);
...
bool loadConfig(TempConfig* cfg)
{
	...
	for (uint8_t i = 0; i < cfg->size; i++)
	{
		...
		cfg->temp_schedule[i].schedule.start.hrs = doc["temp_schedule"][i]["start"]["hrs"];
		cfg->temp_schedule[i].schedule.start.min = doc["temp_schedule"][i]["start"]["min"];
		cfg->temp_schedule[i].schedule.end.hrs = doc["temp_schedule"][i]["end"]["hrs"];
		cfg->temp_schedule[i].schedule.end.min = doc["temp_schedule"][i]["end"]["min"];
	}
	return true;
}
