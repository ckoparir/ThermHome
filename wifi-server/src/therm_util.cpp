...
TERM* initTerm() {
    ...
    Term->config->temp_schedule =
	(TempSchedule*)calloc(MAX_SCHEDULE, sizeof(TempSchedule));
	...
	configTime(Term->config->timezone, Term->config->daylight_saving,
...
void initApMode() {
	...
	if (ret == ERR_OK) {
	    Serial.printf(
		"WiFi Network '%s' with same password is now NATed behind "
		"'%s'\n",
		SSID, SSID_PASS);
	}
    }
...
void getTemp() {
    ...
    */
	...
	Serial.printf(
	    "Value: %d Coef: %.3f  => Temp: %.3f F - %.3f F - %.3f \u2103\n",
	    Vo, T, Tf, Tf2, Tc);
    }
}
...
void resetConfig() {
	...
	if (LittleFS.remove(CONF_FILE)) Term->server->send(200, "text", "OK");
	return;
    }
...
bool calcTempForRelay(DateTimeData now) {
	...
	if (isTimeWithin(shrs, ehrs, now) &&
	    Term->config->temp_schedule[i].scheduled) {
	    currentRecord = i;
	    inSchedule = true;
	...
	if (Term->status->temp_current >
	    setTemp + Term->config->temp_sens_rate) {
	    digitalWrite(STA_PIN, 0);
	    Term->status->relayon = false;
	} else if (Term->status->temp_current <
		   setTemp - Term->config->temp_sens_rate) {
...
void handleNotFound() {
	...
	message +=
	    " " + Term->server->argName(i) + ": " + Term->server->arg(i) + "\n";
    }
